<template>
  <div class="rounded overflow-hidden shadow-md">
    <div class="px-6 py-4">
      ...
      >{{ $tc("poll.participants", users.length, { count: users.length }) }}</p>
      <p class="text-gray-700 text-base">{{ description }}</p>
    </div>
    <div class="flex justify-between flex-wrap px-6 py-4">
      <router-link
        v-if="users.includes(userId)"
        :data-testid="isActive ? `vote-${questionSlug}` : `results-${questionSlug}`"
      ...
      >{{ $t("global.share") }}</button>
      <button
        :data-testid="isActive ? `close-${questionSlug}` : `reOpen-${questionSlug}`"
        v-if="owner === userId"
      ...
      >{{ isActive ? $t("global.close") : $t("global.reOpen") }}</button>
